...
do
	...
	echo "first status is $status"
        timestamp=$(zgrep $ccid $logfile | zgrep  'Response from XL' | cut -d "[" -f4 | cut -d "]" -f1)
        echo $timestamp  >> xltran.txt
...
#fi
		...
		*$response2*)
		...
		#	xltransid=$(zgrep $ccid $logfile | zgrep  'Response to MIFE' | sed -e 's/.*serverReferenceCode\(.*\)serverReferenceCode.*/\1/' | sed -e 's/[^a-zA-Z*0-9-]/ /g;s/  */ /g')
			xltransid=$(zgrep $ccid wso2carbon.log.2019-09-21.gz | zgrep  'Response to MIFE' | sed -e 's/.*serverReferenceCode\(.*\)serverReferenceCode.*/\1/' | sed -e 's/[^a-zA-Z*0-9-]/ /g;s/  */ /g' | sed "s/ //g")
			echo "xltransid is $xltransid"
			#xltranid=$xltransid
			#echo "xltranid is $xltranid"
...
awk '{printf "%s%s",$0,NR%4?"\t":RS}' xltran.txt  > $outputfile
